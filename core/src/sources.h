...
// Manages the capture subject — WGC monitor_capture for the desktop, WGC
// window_capture for the foreground game (no injection, works with
// anti-cheat / borderless games) — plus the configured audio sources, all as
// scene items of the main scene. Also owns WASAPI device enumeration.
//
...
public:
  ...
  // (Re)create the monitor + window sources from config_.capture and
  // re-evaluate the subject for the current mode. Safe to call repeatedly.
  ...
  // GameDetect foreground callback (poll thread): known-game flag, exe, name,
  // pid, window title/class, and whether the window is true fullscreen.
  void onGameForegroundChanged(bool known, const std::string& exe, const std::string& name, uint32_t pid,
...
private:
  ...
  // Capture sources + scene items (one visible at a time).
  ...
  Subject pending_; // debounced switch target (another game)
